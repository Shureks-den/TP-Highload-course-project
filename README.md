...
Будем считать, что запрос с проверкой валидности авторизационного токена совершается раз в сутки. Средний рпс:
...
Медиафайлы:
Используем S3 хранилище - **Amazon S3** для хранения изображения. Для аудиофайлов будем использовать собственные сервера, так как хранить около петабайта данных в S3 экономически нецелесообразно.
...
Используем **Nginx** для балансировки нагрузки и в качестве reverse-proxy. В качестве алгоритма балансировки будем использовать round-robin так как он обеспечивает в среднем равномерную балансировку для неблокирующих соедениний, чем и является golang.
...
При использовании CDN пользователь переадресуется к географически ближайшему кэширующему серверу в составе CDN, благодаря чему доставка статического контента происходит гораздо быстрее.[13]

### Расчет
Используя информацию о rps на медиафайлы (из пункта 2), а также информацию о распределение пользователей по частям света получим (из пункта 1):

Средний rps на стриминг песни 205 556 rps:
Пиковая сетевая нагрузка 6,57 тбит/с, округлим до 7.5 тбит/с, чтобы был запас по нагрузке.

| Европа      | Северная Америка | Латинская Америка | Остальной мир |
|-------------|------------------|-------------------|---------------|
| 34%         | 24%              | 22%               | 20%           |
| 124 млн     | 87,5 млн         | 80 млн            | 73 млн        |
| 69 889 rps  | 49 333 rps       | 45 222 rps        | 41 111 rps    |
| 2,55 тбит/c | 1,8 тбит/c       | 1,65 тбит/c       | 1,5 тбит/c    |

Из этой таблицы видно, что наибольший трафик приходится на Европу, а остальной трафик распределяется примерно равномерно между остальными частями света. Информация из этой таблицы позволит определить оптимальные места для выбора географического положения дата-центров.

С учетом информации о распределении пользователей и географической карты дата-центров [11] для размещения серверов выберем следующие города (4 в европейских городах, по 2 из северной и латинской Америки и 3 в разных частях Азии и Океании):
  * Москва (Россия)
  * Франкфурт (Германия)
...
Каждому из кластеров понадобится балансировщик нагрузки с дополнительной репликой для отказоустойчивости. Из последних двух таблиц получим:
16 балансировщиков на каждый евпропейский дата-центр, 23 на северо-американские, 21 латино-американские, 13 остальные. 

Весь трафик будет приходить по https, для большей приватности данных пользователей, более того, использовать http в 2022 году для крупного сервиса невозможно:
1) Браузеры будут выдавать огромное уведомление, что сайт не является безопасным == часть пользователей уйдет;
2) Невозможно добавить приложение в App Store и Google Store == часть потенциальной аудитории упущена.
Это все ведет к тому, что балансировщики нагрузки также будут должны обрабывать https соединение и проводить ssl termination. Что увеличит нагрузку на cpu. Использование всех современных требований к ssl, увеличивает нагрузку на cpu в среднем в 2-3 раза. Поэтому необходимо увеличить производительность по cpu возьмем наиболее произодительные процессоры на данных момент - AMD EPYC™ 7742. Этот процессор обладает, 64 ядрами, которые поддерживают мультипоточность и превосходит процессоры Intel при меньшей цене. [14]
...
Текстовой информации у нас тоже немного ~500гб, но для повышения производительности можно поставить такой же объем оперативной памяти, для более эффективного кеширования. Одного диска на 4тб хватит для RAID 10. Для сохранности данных используем 1 мастера для записи и 2 слейвов для чтения. Слейвы будут находиться в городах указанных в таблице, в то время как мастера разместим лишь в одном дата центре.

#### О репликации базы данных

Когда вы данные изменяются в базе, все изменения пишутся во Write-Ahead Log, или WAL. После записи в WAL СУБД делает системный вызов fsync, благодаря чему данные попадают сразу на диск, а не висят в где-то в кэше файловой системы. Таким образом, если взять и обесточить сервер, при следующей загрузке СУБД прочитает последние записи из WAL и применит к базе данных соответствующие изменения.
...
| Технология  |  RAM  |  Диск(и) | CPU |           Количество серверов |
|-------------|:-----:|---------:|----:|------------------------------:|
| Аудиофайлы  | 128гб | 4Тб * 24 |  32 |         52 * 11(каждый город) |
| Изображения | 128гб |      4Тб |  32 |          2 * 11(каждый город) |
| PostgreSql  | 512гб |      4ТБ |  32 |       1+ 2 * 11(каждый город) |
| Redis       | 256гб |    128гб |  16 |      (2+2) * 11(каждый город) |
| Backend     | 256гб |    128гб |  32 | (1+1+1+1)*2 *11(каждый город) |
| Nginx       | 256гб |    128гб |  64 | 191+(1+1)*2 *11(каждый город) |
...
## Источники
    ...
    13. https://habr.com/ru/company/selectel/blog/463915/
    14. https://www.amd.com/ru/products/cpu/amd-epyc-7742
    15. https://eax.me/postgresql-replication/
